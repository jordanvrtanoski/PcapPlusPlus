...
namespace pcpp
{
	...
	std::vector<sctp_gap_ack_block> SctpChunk::getNrSackGapBlocks() const
	{
		...
		{
			auto* block =
			    reinterpret_cast<const sctp_gap_ack_block*>(reinterpret_cast<const uint8_t*>(m_Data) + blockOffset);
	...
	std::vector<uint32_t> SctpChunk::getNrSackDupTsns() const
	{
		...
		size_t tsnOffset = sizeof(sctp_nr_sack_chunk) + (numGapBlocks * sizeof(sctp_gap_ack_block)) +
		...
		{
			uint32_t tsn =
			    be32toh(*reinterpret_cast<const uint32_t*>(reinterpret_cast<const uint8_t*>(m_Data) + tsnOffset));
